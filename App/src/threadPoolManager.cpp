...
void ThreadPoolManager::handleTaskFinished(const int &task_id,
    ...
    switch (task_id) {
        ...
        case PALLET_MODEL_ADD:
            ...
            break;
        case PALLET_QUEUE_GET_ALL:
            // //qDebug() << "Task is of type ModelTask";
            // Perform actions specific to ModelTask
            emit addModelTaskCompleted(result);
            break;
        case PALLET_BUFFER_GET_ALL:
            // //qDebug() << "Task is of type ModelTask";
            // Perform actions specific to ModelTask
            emit addModelTaskCompleted(result);
            break;
...
void ThreadPoolManager::handleVectorTaskFinished(const int &task_id,
                                                 const std::vector<std::string> &result) {
    switch (task_id) {
        case PALLET_QUEUE_GET_ALL:
            // //qDebug() << "Task is of type ModelTask";
            // Perform actions specific to ModelTask
            emit getAllQueueCompleted(result);
            break;
        case PALLET_BUFFER_GET_ALL:
            // //qDebug() << "Task is of type ModelTask";
            // Perform actions specific to ModelTask
            emit getAllBufferCompleted(result);
            break;

        default:
            //qDebug() << "VectorTask is of an unknown or base type AsyncTask";
            break;
    }
}
...
void ThreadPoolManager::executeTask(AsyncTask *task) {
    if (task) {
        ...
        bool finishedVectorConnection = connect(
            task, &AsyncTask::vectorTaskFinished, this,
